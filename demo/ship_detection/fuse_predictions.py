...
def fuse_prediction(pkl_paths, output, json_anno):
    ...
    for img in anno['images']:
        fname = img["file_name"]
        img = cv2.imread(f'/home/yuchunli/_DATASET/ship-detection/test/{fname}')
        h, w, c = img.shape
    ...
    for i in range(num_imgs):
        ...
        fused_box, fused_score, fused_label = weighted_boxes_fusion(
            boxes, scores, labels, weights=None, iou_thr=0.5, skip_box_thr=0.0001
        )
        fused = np.concatenate([fused_box * np.array(img_dims[i]), fused_score.reshape(-1, 1)], axis=1)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--json", type=str, default="/home/yuchunli/_DATASET/ship-detection-coco-full/annotations/instances_test.json")
