...
from mmcv.runner import (RUNNERS, EpochBasedRunner, IterBasedRunner,
                         IterLoader, get_dist_info)
from mmcv.runner.utils import get_host_info
from torch.utils.data.dataloader import DataLoader


@RUNNERS.register_module(force=True)
class EpochBasedRunnerWithCancel(EpochBasedRunner):
    """Simple modification to EpochBasedRunner to allow cancelling the training
    during an epoch.

    A stopping hook should set the runner.should_stop flag to True if stopping
    is required.
    """
    ...
    def train(self, data_loader: DataLoader, **kwargs):
        ...
        if self.distributed:
            time.sleep(
                2
            )  # Prevent possible multi-gpu deadlock during epoch transition
...
class IterBasedRunnerWithCancel(IterBasedRunner):
    ...
    def main_loop(self, workflow: List[tuple],
                  iter_loaders: Sequence[IterLoader], **kwargs):
        ...
        while self.iter < self._max_iters:
            for i, flow in enumerate(workflow):
                ...
                if not isinstance(mode, str) or not hasattr(self, mode):
                    raise ValueError(
                        f'runner has no method named "{mode}" to run a workflow'
                    )
