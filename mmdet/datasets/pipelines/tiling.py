...
class Tile:
    ...
    def __cache_tiles(self):
        ...
        for i, tile in enumerate(self.__tiles):
            tile['tile_path'] = osp.join(self.tmp_folder,
                                         "_".join([str(i), tile['uuid'],
                                                   tile['ori_filename']]))
    ...
    def __gen_tiles_single_img(self, result: Dict,
    ...
    def __tile_ann_assignment(self, tile_result: Dict, tile_box: np.ndarray,
    ...
    def __tile_boxes_overlap(self, tile_box: np.ndarray,
    ...
    def __tile_nms(self, bbox_results: List[List], segm_results,
        ...
        for i, result in enumerate(zip(bbox_results, segm_results)):
            ...
            segm_results[i] = [list(segms[labels == i])
    ...
    def __merge(self, results: List[List]) -> List[List]:
        ...
        merged_seg_results = [[[] for _ in range(num_classes)]
                              for _ in range(self.num_images)]
    ...
    def evaluate(self, results, **kwargs) -> Dict[str, float]:
        ...
        if 'dump_path' in kwargs:
            path = kwargs.pop('dump_path')
            if path != None and osp.exists(osp.dirname(path)):
                mmcv.dump(merged_results, path)
