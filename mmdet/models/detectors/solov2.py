...
class SOLOv2(SingleStageInstanceSegmentor):
    ...
    def aug_test(self, imgs, img_metas, rescale=False):
        ...
        aug_labels = torch.stack(aug_labels)
        aug_scores = torch.stack(aug_scores)
        aug_bboxes = mask2bbox(aug_masks)

        # NOTE: hard core this for now
        nms_cfg = {
            'type': 'nms',
            'iou_threshold': 0.6,
        ...
        _, keep = batched_nms(aug_bboxes, aug_scores, aug_labels, nms_cfg)

        results = InstanceData()
        results.masks = aug_masks[keep]
        results.labels = aug_labels[keep]
        results.scores = aug_scores[keep]
        results = self.format_results(results)
        return [results]
